const ALLOWED_EVENTS = new Set(['plugin_install', 'plugin_start', 'status_update']);
...
async function buildSummary(env) {
  ...
  const [totalInstalls, running1, running7, running30, versions, platforms, statusCounts, daily] = await Promise.all([
...
<head>
  ...
  <style>
    ...
    @media (max-width: 880px) { .grid { grid-template-columns: 1fr; } main { padding: 14px; } }
  </style>
</head>
<body>
  <header>
  ...
  <main>
    <div class="grid">
      ...
      <div class="card"><h2>Running 24h</h2><div class="metric" id="running1">-</div></div>
      <div class="card"><h2>Running 7d</h2><div class="metric" id="running7">-</div></div>
      <div class="card"><h2>Running 30d</h2><div class="metric" id="running30">-</div></div>
    </div>
